...
#The following must be syncrhonised with the rest of the SYSNAME scripts
bottomCode = 0x4400
topCode = 0xc3fe
...
def startParsing(outputFile,counter):
    ...
    os.system("../toolchain/compiler/msp430-gcc-9.2.0.50_linux64_original/bin/msp430-elf-objdump -D -z -j .text -j .lower.text appWithNoMetadata.out > code.tmp")
    ...
    os.system("../toolchain/compiler/msp430-gcc-9.2.0.50_linux64_original/bin/msp430-elf-objdump -D -z -j .appText appWithNoMetadata.out > code.tmp2")
    ...
    while line:
        ...
        #Temporary label was found
            ...
            newEntry = ".LLee"+str(globalCounter)+str(counter) + ":\n"

            # Add entry to dictionary of labels
            labels.append({"name":".LLee"+str(globalCounter)+str(counter),"address":match.group(1),"used":False})
            counter+=1
