...
int main() {
  klee_make_symbolic(mockReads,
                     sizeof(mockReads),
                     "mouse_adc_trace");

  klee_make_symbolic(&lastSwitchState,
                     sizeof(lastSwitchState),
                     "mouse_initial_switch_state");

  klee_make_symbolic(&lastMouseLeftState,
                     sizeof(lastMouseLeftState),
                     "mouse_initial_left_state");

  klee_make_symbolic(&lastMouseRightState,
                     sizeof(lastMouseRightState),
                     "mouse_initial_right_state");

  for (int i = 0; i < 200; ++i) {
      klee_assume(mockReads[i] >= -200);
      klee_assume(mockReads[i] <= 200);
  }
  for(int i=0; i<100; i++){
